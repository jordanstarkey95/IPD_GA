...
for k=1:length(memory_lengths)
  ...
  for i=1:length(crossover_rates)
    for j=1:length(mutation_rates)
      ...
      filename = strcat('..\ENCODEDcrossover_rate-', num2str(crossover_rate), 'mutation_rate-', num2str(mutation_rate), 'memory_length-', num2str(memory_length), 'number_of_games-', num2str(number_of_games),  '_summary.txt');
      total_best = 0;
      for l=0:(nr_runs-1)
...
set(leg,'FontSize', 14);
saveas(gcf,'average_best_fitness_axelrod.png')